...
void DrivetrainDefaultCommand::Execute()
{
  ...
  //  units::scalar_t rS{frc::ApplyDeadband(rot, 0.02) * kSwerve::robotMaxAngularVelocity.value()};
   ...
   units::meters_per_second_t xS{frc::ApplyDeadband(x, 0.08) * kSwerve::driveMaxSpeed.value()};
   units::meters_per_second_t yS{frc::ApplyDeadband(y, 0.08) * kSwerve::driveMaxSpeed.value()};
   units::radians_per_second_t rS{frc::ApplyDeadband(rot, 0.08) * kSwerve::robotMaxAngularVelocity.value()};
  ...
  m_drivetrain->Drive(yS, -xS, -rS, true,true);
